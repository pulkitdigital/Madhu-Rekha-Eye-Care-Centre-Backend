...
class DischargeTicketPDFService {
  ...
  generateContent(doc, ticketData, clinicProfile) {
    ...
    // Add doctors info - UPDATED FORMAT
  ...
  /**
   * Add doctor information (two columns) - UPDATED FORMAT
   * Shows: Name, Reg. No:, Qualification in sequence
   */
  addDoctorInfo(doc, clinicProfile, contentLeft, contentRight, usableWidth, startY) {
    let y = startY;

    const doctor1Name = clinicProfile.doctor1Name || '';
    const doctor1RegNumber = clinicProfile.doctor1RegNumber || '';
    const doctor1Qualification = clinicProfile.doctor1Qualification || '';
    
    const doctor2Name = clinicProfile.doctor2Name || '';
    const doctor2RegNumber = clinicProfile.doctor2RegNumber || '';
    const doctor2Qualification = clinicProfile.doctor2Qualification || '';

    // Doctor 1 Name (Left side) - BOLD
    doc.fontSize(9).font('Helvetica-Bold');
    doc.text(doctor1Name, contentLeft, y);

    // Doctor 2 Name (Right side) - BOLD
    ...
    // Doctor 1 Reg. No. (Left) - Regular font
    doc.font('Helvetica').fontSize(8);
    const doctor1RegText = doctor1RegNumber ? `Reg. No: ${doctor1RegNumber}` : '';
    doc.text(doctor1RegText, contentLeft, y);

    // Doctor 2 Reg. No. (Right) - Regular font
    const doctor2RegText = doctor2RegNumber ? `Reg. No: ${doctor2RegNumber}` : '';
    doc.text(doctor2RegText, contentLeft, y, {
      width: usableWidth,
      align: 'right'
    });
    ...
    // Doctor 1 Qualification (Left) - Regular font
    doc.fontSize(8);
    doc.text(doctor1Qualification, contentLeft, y);

    // Doctor 2 Qualification (Right) - Regular font
    doc.text(doctor2Qualification, contentLeft, y, {
      width: usableWidth,
      align: 'right'
    });
